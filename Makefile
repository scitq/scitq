...
# `copy-docs` also depends on `tgz-python-src` and `stub-public` because
# both python-src.tgz and server/public/ are //go:embed inputs
# (python/embed.go and server/http.go) that are .gitignored and
# regenerated by `make install`. Every place that runs `go build ./...`
# — including CI — invokes `make copy-docs` first, so folding these
# fallbacks in here means embed patterns are always satisfied without
# each caller having to remember them by hand. `tgz-python-src` is pure
# shell + tar; `stub-public` writes a placeholder index.html only when
# server/public/ is missing (leaves a real vite build untouched). The
# actual UI build (`ui-embed`) is run separately in CI's build job so
# test jobs stay Go-only.
copy-docs: tgz-python-src stub-public
	@mkdir -p server/docs
...
# stub-public creates server/public/index.html only if it doesn't exist
# yet. The Go compiler needs at least one file matching the
# //go:embed all:public pattern; test jobs don't touch the UI endpoints
# so a placeholder is fine. `ui-embed` runs `rm -rf server/public/*`
# before copying vite's real output, so if it runs later this stub gets
# replaced cleanly.
.PHONY: stub-public
stub-public:
	@if [ ! -f server/public/index.html ]; then \
		mkdir -p server/public; \
		echo '<!DOCTYPE html><html><body>SCITQ UI placeholder — run `make ui-embed` for the real build.</body></html>' > server/public/index.html; \
		echo "✓ Created server/public/index.html (stub)"; \
	fi
