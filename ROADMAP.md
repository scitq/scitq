...
[ ] job-engine restart-orphan reconciliation — jobs in `R` when the server dies stay `R` forever (no in-flight resume, no startup sweep). Currently harmless for delete jobs because the worker still gets soft-deleted by the cloud API before the crash in most cases, but it (a) accumulates zombie job rows, (b) shows up as an alert on `scitq_deletion_jobs_stuck` before the truthful "check the worker actually still exists" fix that was added 2026-09-17. Simple sweep at startup: `UPDATE job SET status='F', modified_at=NOW() WHERE status='R'` (safe because the previous server process no longer holds the goroutine that would drive them). Better: also enqueue a fresh retry for `action='D'` jobs whose target worker is still alive so a genuinely-hung delete doesn't get abandoned.
