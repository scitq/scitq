...
CREATE TABLE IF NOT EXISTS worker (
    ...
    status CHAR(1) NOT NULL DEFAULT 'I', -- (O: Offline, I: Installing, R: Ready, P: Paused, F: Failing, Q: Quarantined (pause+offline), L: Lost (failing+offline),  'D': Deleting)
    stats JSONB DEFAULT '{}',
    last_ping TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS task (
    ...
    status CHAR(1) NOT NULL DEFAULT 'P',  -- (P: Pending, A: Assigned, C: Accepted, D: Downloading, O: on hold (ready for exec in the worker, waiting), R: Running, U: Uploading (after success), V: Uploading (after failure), S: Succeeded, F: Failed, Z: suspended, X: canceled, W: waiting), I: inactive (not yet scheduled)
